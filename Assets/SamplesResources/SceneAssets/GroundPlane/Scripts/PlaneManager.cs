   ...
   // ProductPlacement m_ProductPlacement;
    ...
    void Update()
    {
        ...
        SetSurfaceIndicatorVisible(
            ...
            (planeMode == PlaneMode.GROUND) && !AstronautIsPlaced);
    }
    ...
    {
       // Debug.Log("HandleAutomaticHitTest()_____");
    ...
    {
        Debug.Log("HandleInteractiveHitTest()_1");
        if (result == null)
        {
        ...
        if (!m_GroundPlaneUI.IsCanvasButtonPressed() && !AstronautIsPlaced)
        {
            ...
            switch (planeMode)
            {
                case PlaneMode.GROUND:
                    ...
                    m_PlaneAugmentation.GetComponent<LeanRotateCustomAxis>().enabled = true;
                    m_PlaneAugmentation.GetComponent<LeanTranslate>().enabled = true;
                    m_PlaneAugmentation.GetComponent<LeanScale>().enabled = true;
                    m_PlaneAugmentation.GetComponent<TestRepeated>().enabled = true;
                    AstronautIsPlaced = true;
                    m_GroundPlaneUI.gameObject.SetActive(false);
                   // StartCoroutine(YuoTubePlayer.Play());
                   // YuoTubePlayer.StartVideoPortal();
                    break;
    ...
    {
       // Debug.Log("SetSurfaceIndicatorVisible()="+isVisible);
